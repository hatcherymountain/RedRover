...
public class Numbers {
	...
	private void addTestFlow(int aid,int uid,int id)
	{
		if(eos.active())
		{
			...
			try {
				...
				String sql = "insert into rr_flows values(null,"+aid+",0,0,'"+phone+"','test','"+output+"',0,0)";
				s = c.createStatement();
				s.execute(sql);
	...
	public void customerAssignNumber(String nid)
	{
		...
		if(eos.isCustomerAdmin()||eos.isAdmin()||eos.isContributor())
		{
			...
			try
			{
				...
				//eos.log(sql);
				s.execute(sql);
				eos.audit().audit("Number " + id + " assigned to account "+ aid + "","customerAssignNumber","Numbers","customerAssignNumber");
	...
	public String getAvailableNumbersAsSelection()
	{
		...
		ArrayList<Number> lst = getAvailableNumbers(); int size = lst.size();
