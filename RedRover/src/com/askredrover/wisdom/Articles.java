...
public class Articles {
	...
	public void linkFile(String parentid, String fileid, String checked) {
		...
		try {
			...
			eos.log("Errors linking or delinking files from an article. Err:" + e.toString(), "Articles", "linkFile",
					2);
		} finally {
			eos.cleanup(c, s);
		}
	...
	public boolean isArticleLinked(String parentid, String aid) {
		...
		try {
			...
			eos.log("Errors determining whether article linked to a parent article. Err::" + e.toString(), "Articles",
					"isArticleLinked", 2);
		} finally {
			eos.cleanup(c, s, rs);
		}
	...
	public boolean isFileLinked(String parentid, String fid) {
		...
		try {
			...
			eos.log("Errors determining whether FILE linked to a parent article. Err::" + e.toString(), "Articles",
					"isFileLinked", 2);
		} finally {
			eos.cleanup(c, s, rs);
		}
	...
	public void link(javax.servlet.http.HttpServletRequest r) {
		if (eos.isAdmin()) {
			...
			try {
				...
				String aid = r.getParameter("articleid");
				int iAid = eos.d(aid);
				if (iAid > 0) {
