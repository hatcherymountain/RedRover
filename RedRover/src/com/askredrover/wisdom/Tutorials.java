...
public class Tutorials {
			...
			try {
				...
				if (cid > 0) { // Need categories!
					...
					String sql = "insert into wisdom_tutorials values(null," + eid + "," + aid + "," + sid + ",'"
							+ title + "','" + description + "'," + cid + ",'" + today + "',0," + iRole + ",'" + youtube
							+ "')";
		...
		if (eos.isAdmin()) {
			...
			try {
				...
				if (title.length() > 0) {
					...
					String sql = "insert into wisdom_tutorial_step values(null," + id + "," + indx + ",'" + title
							+ "','','',0)";
					s.execute(sql);
	...
	public void update(javax.servlet.http.HttpServletRequest r) {
		if (eos.isAdmin()) {
			...
			try {
				...
				String storeid = r.getParameter("storeid");
				int sid = eos.d(storeid);
				...
				String category = r.getParameter("categoryid");
	...
	public ArrayList<Step> steps(String tid) {
		...
		try {
			...
			s = c.createStatement();
			int id = eos.d(tid);
			String sql = "select stepid,tutid,indx,title,description,url,fileid from wisdom_tutorial_step where tutid="
					+ id + " order by indx asc";
			rs = s.executeQuery(sql);
			while (rs.next()) {
				int sid = rs.getInt(1);
				int t = rs.getInt(2);
	...
	private ArrayList<Tutorial> getTutorialsForUser(int cid) {
		...
		try {
			...
			s = c.createStatement();
			String sql = "select tutid,accountid,storeid,title,description,added,status,roleid,youtube from wisdom_tutorials where eid="
			...
			while (rs.next()) {
				...
				String yt = rs.getString(9);

				Tutorial tut = new TutorialObject(tid, eid, aid, sid, t, d, cid, added, status, role, yt);
	...
	public Tutorial get(String tutid) {
		...
		if (eos.active()) {
			...
			try {
				...
				s = c.createStatement();
				String sql = "select tutid,eid,accountid,storeid,title,description,categoryid,added,status,roleid,youtube from wisdom_tutorials where eid="
						+ eid + " and tutid=" + id + "";
				...
				while (rs.next()) {
					...
					String yt = rs.getString(11);

					t = new TutorialObject(tut, e, a, sid, title, desc, cid, added, status, rid, yt);
		...
		if (eos.active()) {
			...
			try {
				...
				while (rs.next()) {
					...
					Tutorial t = new TutorialObject(tut, e, a, sid, title, desc, cid, added, status, rid, yt);
					lst.add(t);
