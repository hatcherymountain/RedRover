...
public class Files {
	...
	/**
	 * Will work in correspondence with eos.files removal, 
	 * but for now we just remove from the db.
	 * @param fileid
	 */
		...
		if(eos.isAdmin() || eos.isContributor())
		{
			...
			try { 
				...
				if(id  > 0)
				{
					...
					/** Get the values **/
					String storeid = r.getParameter("companyid"); int sid = eos.d(storeid);
					String catid   = r.getParameter("catid");     int cid = eos.d(catid);
					...
					String tag     = r.getParameter("tag"); tag = com.eos.Eos.clean(tag); tag = com.eos.utils.Strings.absoluteTruncation(tag, 50);
					String status  = r.getParameter("status"); int iStatus = com.eos.utils.Strings.getIntFromString(status);
					String roleid  = r.getParameter("roleid"); int rid = eos.d(roleid);
	 ...
	 * @return boolean
	 */
	private boolean passSecurityCheck(File file)
	{
	...
	public ArrayList<com.eos.files.File> accountFiles(String category, String store) { 
		...
		ArrayList<com.eos.files.File> initList = eos.files().getAccountFilesWithCategory(); //We want files with a category ONLY 
