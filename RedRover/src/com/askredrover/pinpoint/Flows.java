...
public class Flows {
	...
	/**
	 * When a new account comes on board, we go through a simple test flow. This
	 * flow is later removed ONCE team members are added to the system
	 * 
	 * @param nid
	 */
		...
		if (eos.active()) {
			...
			try {
				...
				if (phone != null) {
					...
					String sql = "insert into rr_flows values(null," + aid + ",0,0,'" + phone
							+ "','test','Welcome to RedRover!',0)";
					s.execute(sql);
				}
